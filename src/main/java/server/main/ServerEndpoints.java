...
public class ServerEndpoints {
	...
	public @ResponseBody ResponseEnvelope<?> mapExchange(@PathVariable UniqueGameIdentifier gameID,
		...
		if (gameController.checkIfPlayerIsInTheGame(gameController.getGame(uniqueGameID), playerID) == false)
			throw new NotEnoughPlayersException("Player not found",
					"player with this ID is not registered for this game");

		if (gameController.checkNumberOfRegisteredPlayers(uniqueGameID) == false)
			throw new NotEnoughPlayersException("NotEnoughPlayersException",
					"There is not enoughPlayers or the is not registered for this game");

		if (gameController.checkIfPlayerAlereadySentMap(uniqueGameID, playerID) == true)
			throw new PlayerAlreadySentMapException("PlayerAlreadySentMapException",
					"player has already sent half map");
